...
@Component({
  selector: 'app-graficas-no-operativos-nacional',
  templateUrl: './graficas-no-operativos-nacional.component.html',
  styleUrls: ['./graficas-no-operativos-nacional.component.css']
})
export class GraficasNoOperativosNacionalComponent implements OnInit {
...
private Graficarmesactual() {
  this.logicaService.getProcentajeCumplimietoZonasSegmentos().subscribe(
    (datos) => {
      console.log('Estos son los datos que me estadevolviendo el servicio de logica en el metodo de zonas', datos)
...
actualizarGrafica1mesactualConDatos(datos: any) {
  ...
  // Asegúrate de que las propiedades sean correctas y coincidan con las reales
  const pacifico = datos[3].Pacífico;
  const norte = datos[3].Noreste;
  const sur = datos[3].Sureste;
  const centro = datos[3].Centro;
