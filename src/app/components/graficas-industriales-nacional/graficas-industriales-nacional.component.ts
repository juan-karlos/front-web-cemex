...
@Component({
  selector: 'app-graficas-industriales-nacional',
  templateUrl: './graficas-industriales-nacional.component.html',
  styleUrls: ['./graficas-industriales-nacional.component.css']
})
export class GraficasIndustrialesNacionalComponent implements OnInit {
...
actualizarGrafica1mesactualConDatos(datos: any) {
  ...
  // Asegúrate de que las propiedades sean correctas y coincidan con las reales
  const pacifico = datos[6].zona;
  const norte = datos[4].zona;
  const sur = datos[5].zona;
  const centro = datos[7].zona;
  const nacional = datos[7].nacional;
