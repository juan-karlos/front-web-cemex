...
@Component({
  selector: 'app-graficas-suministro-nacional',
  templateUrl: './graficas-suministro-nacional.component.html',
  styleUrls: ['./graficas-suministro-nacional.component.css']
})
export class GraficasSuministroNacionalComponent implements OnInit {
...
actualizarGrafica1mesactualConDatos(datos: any) {
  ...
  // Asegúrate de que las propiedades sean correctas y coincidan con las reales
  const pacifico = datos[22].zona;
  const norte = datos[20].zona;
  const sur = datos[21].zona;
  const centro = datos[23].zona;
  const nacional = datos[23].nacional;
