...
@Component({
  selector: 'app-graficas-transporte-pacifico',
  templateUrl: './graficas-transporte-pacifico.component.html',
  styleUrls: ['./graficas-transporte-pacifico.component.css']
})
export class GraficasTransportePacificoComponent implements OnInit {
...
actualizarGrafica1mesactualConDatos(datos: any) {
  ...
  // Asegúrate de que las propiedades sean correctas y coincidan con las reales
  const pacifico = datos[6].Pacífico;
  const norte = datos[6].Noreste;
  const sur = datos[6].Sureste;
  const centro = datos[6].Centro;
