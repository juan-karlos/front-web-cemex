...
@Component({
  selector: 'app-graficas-operaciones-noreste',
  templateUrl: './graficas-operaciones-noreste.component.html',
  styleUrls: ['./graficas-operaciones-noreste.component.css']
})
export class GraficasOperacionesNoresteComponent implements OnInit {
...
actualizarGrafica1mesactualConDatos(datos: any) {
  ...
  // Asegúrate de que las propiedades sean correctas y coincidan con las reales
  const pacifico = datos[4].Pacífico;
  const norte = datos[4].Noreste;
  const sur = datos[4].Sureste;
  const centro = datos[4].Centro;
